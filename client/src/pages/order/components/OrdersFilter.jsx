...
export default function OrdersFilter({ userCategories, setUserCategories }) {
  ...
  const hdlCatClick = (e, catId) => {
    ...
    const newSelections = [...userCategories];
  ...
  return (
    <>
      ...
      <div className="block mt-0">
        ...
        <button
          className={`button is-fluid mb-4 ${defaultSort ? 'is-primary' : date && 'is-primary'}`}
          onClick={hdlDateFilter}
        >
          <span>По дате</span>
          <span>
            <i
              className={`fa-solid ${defaultSort ? 'fa-angle-down' : date === 'up' ? 'fa-angle-up' : 'fa-angle-down'}`}
            ></i>
          </span>
        </button>
        <button
          className={`button is-fluid mb-4 ${price && !date && 'is-primary'}`}
          onClick={hdlPriceFilter}
        >
          <span>По стоимости</span>
          <span>
            <i
              className={`fa-solid ${price === 'up' ? 'fa-angle-up' : 'fa-angle-down'}`}
            ></i>
          </span>
        </button>
      </div>
        ...
        {userCategories.length > 0 &&
          userCategories.map((cat) => {
            return cat.selected ? (
              <button
                key={cat.id}
                className="button is-fluid mb-4 is-left is-link"
                onClick={(e) => {
                  hdlCatClick(e, cat.id);
                }}
              >
                <span>
              ...
              <button
                key={cat.id}
                className="button is-fluid mb-4 is-left"
                onClick={(e) => {
                  hdlCatClick(e, cat.id);
                }}
              >
